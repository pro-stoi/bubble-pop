...
class ApiClient {
  ...
  // api.js - в методе updateStats()

async updateStats(data) {
    console.log('📌 [API] updateStats() вызван');
    console.log('📌 [API] data.lastPlay на входе:', data.last_play);
    console.log('📌 [API] Весь data:', data);
...
// js/core/api.js - метод saveToTop

async saveToTop(data) {
    console.log('📌 [API] saveToTop() вызван');
    console.log('📌 [API] Данные для сохранения:', data);
    
    var result = await this.request('/top/save', {
        method: 'POST',
        body: data
    });
    
    console.log('📌 [API] Ответ от сервера:', result);
    return result;
}
