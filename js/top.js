...
console.log('🔥 [TOP] Файл загружен');

document.addEventListener('DOMContentLoaded', function() {
    console.log('🔥 [TOP] DOM загружен');
    ...
    async function loadData() {
        console.log('🔥 [TOP] Загружаем данные через repository');
        
        tbody.innerHTML = '<tr><td colspan="5" class="top-empty">⏳ Загрузка...</td></tr>';
        myRankRow.innerHTML = '<span class="not-in-top">⏳ Загрузка...</span>';
        
        try {
            var data = await repository.loadTop();
            console.log('🔥 [TOP] Получено записей =', data ? data.length : 0);
            
            if (!data || data.length === 0) {
                console.warn('🔥 [TOP] Данных нет');
                tbody.innerHTML = '<tr><td colspan="5" class="top-empty">🎯 Сыграйте первую игру!</td></tr>';
                myRankRow.innerHTML = '<span class="not-in-top">🎯 Сыграйте первую игру!</span>';
                return;
            }
            ...
            console.log('🔥 [TOP] Отрисовываем', players.length, 'игроков');
            render();
            console.log('✅ [TOP] Готово!');
            
        } catch (error) {
            console.error('❌ [TOP] Ошибка:', error.message);
            
            var cached = localStorage.getItem('cachedTop');
            if (cached) {
                try {
                    players = JSON.parse(cached);
                    console.log('🔥 [TOP] Используем кеш,', players.length, 'игроков');
                    render();
                    return;
                } catch (_) {}
            }
    ...
    console.log('🔥 [TOP] Запускаем загрузку');
    loadData();
});