...
class BonusManager {
    ...
    updateUI() {
        ...
        for (const type of types) {
            ...
            btn.innerHTML = `
                ...
                <span class="bonus-label">${labels[type]}</span>
    ...
    updateProgress() {
        ...
        const types = ['pink', 'blue', 'green', 'yellow', 'red'];
        const icons = {
            pink: '🟣',
            blue: '🔵',
            green: '🟢',
        ...
        for (const type of types) {
            ...
            if (isActive && this.currentColor === type) {
                item.classList.add('active');
            }
            
            item.innerHTML = `
                <span class="progress-icon">${icons[type]}</span>
                <div class="progress-bar">
                    <div class="progress-fill" style="width: ${progress * 100}%; background: ${this.bonuses[type]?.color || '#fff'};"></div>
                </div>
                <span class="progress-count">${count}/${this.requiredHits}</span>
            `;
