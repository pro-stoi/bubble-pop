class Game {
    constructor(canvas) {
        ...
        // ===== НОВАЯ МЕХАНИКА =====
        this.pendingScore = 0;          // Накопленные очки
        this.multiplier = 1;            // Текущий множитель
        this.lastPopTime = 0;           // Время последнего попадания
        this.comboTimeout = 2000;       // 2 секунды на комбо
        this.isProcessing = false;      // Флаг обработки накопленных очков
        this.scorePopups = [];          // Всплывающие надписи (+5 ×3!)
    ...
    spawnBubble() {
        // Разные типы пузырьков
       const types = [
    { radius: 12, points: 10, hue: 320, weight: 15 },   // Маленький розовый — 10 очков
    { radius: 18, points: 7, hue: 200, weight: 20 },    // Маленький синий — 7 очков
    { radius: 26, points: 4, hue: 120, weight: 25 },    // Средний зелёный — 4 очка
    { radius: 36, points: 2, hue: 45, weight: 25 },     // Большой золотой — 2 очка
    { radius: 50, points: 1, hue: 0, weight: 15 }       // Огромный красный — 1 очко
];
...
update() {
    ...
    if (this.bubbles.length > 40) {
        // Удаляем самые старые (первые в массиве)
    ...
    // Автоматическое зачисление очков через 2 секунды бездействия
    if (this.pendingScore > 0 && Date.now() - this.lastPopTime > this.comboTimeout) {
        this.flushScore();
    }
}

    // ===== ОТРИСОВКА ВСПЛЫВАЮЩИХ НАДПИСЕЙ =====
    drawPopups(ctx) {
        for (const popup of this.scorePopups) {
            ...
            if (popup.subtext) {
                ...
                ctx.fillStyle = `hsla(${popup.hue}, 100%, 80%, 0.7)`;
